...
double GBT::make_rho0(int level) {
    ...
    if (rho_mode == 2) { // 1 - rho0 decreases with level exponentially
      return 1.0 - (1.0 - rho0) / pow(2.0, level);
    }
    Rcpp::stop("Invalid stopping-probability mode.");
}
...
GBT::GBT(Mat< unsigned int > X,int k, int p, double rho0, int rho_mode, int tran_mode,
        ...
        logrho_mat(NULL), logrho_vec(NULL),
        work_Z(n_s + 1), work_phi(n_s + 1), work_gam(n_s + 1),
        work_unnormalized_rho(n_s + 1), models(NULL), modelscount(NULL) {
...
int GBT::update_node(double *NODE_CURR, int level, INDEX_TYPE I) {
  ...
  vector<double>& Z = work_Z; // log Z(A,t,x), including complete shrinkage
  vector<double>& phi = work_phi; // log phi(A,s,x), including complete shrinkage
...
void GBT::make_prior_logrho_vec(double *logrho_vec, int level, int s) {
  ...
  int t;
  vector<double>& gam = work_gam;
  vector<double>& unnormalized_rho = work_unnormalized_rho;
...
void GBT::sample_subtree(INDEX_TYPE& I,int level,int s,double log_prob) {
    ...
    if (level == k || log(u) <= logrho_vec[n_s]) { // pruned?
      ...
      sample_nodes.push_back(make_pair(
        make_pair(I, level),
        make_pair(std::numeric_limits<double>::infinity(), log_prob)
      ));
	...
	log_cum_Ma_curr = log_exp_x_plus_exp_y(log_cum_Ma_curr, log_Ma_nu);
      ...
      theta = R::rbeta(theta0 * nu + n0, (1 - theta0) * nu + n1);
...
double GBT::get_root_logrho() {
  ...
  const double log_prior_stop = tran_mode == 2
    ? -log(n_s + 1.0)
    : log(make_rho0(0));
  return log_prior_stop + models[0][1+n_s]
    - (get_root_logphi()-models[0][0]*k*log(2.0));
}
