...
// number of observations
    ...
    if (nobs < 1 || p < 1 || Xnew.n_cols != (unsigned int) p ||
        Omega.n_rows != (unsigned int) p || Omega.n_cols != 2) {
      Rcpp::stop("Invalid data, prediction, or sample-space dimensions.");
    }
    if (k < 1) Rcpp::stop("Maximum resolution must be positive.");

    if (k > MAXVAR - 1) {
      Rcpp::warning("Maximum resolution %d exceeds limit %d; using %d.",
                    k, MAXVAR - 1, MAXVAR - 1);
      k = MAXVAR - 1;
    }
...
// outputs to return
    ...
    if (nobs_new > 0) {
      ...
      const double density_scale = arma::prod(a);
      for (size_t i = 0; i < predictive_densities_vec.size(); i++) {
        predictive_densities_vec[i] *= density_scale;
      }
    }
/*
    else {
...
)
{
  ...
  if (nobs < 1 || p_X < 1 || p_Y < 1 || Y.n_rows != (unsigned int) nobs ||
      Xnew.n_cols != (unsigned int) p_X || Ynew.n_cols != (unsigned int) p_Y ||
      Xnew.n_rows != Ynew.n_rows ||
      Omega_X.n_rows != (unsigned int) p_X || Omega_X.n_cols != 2 ||
      Omega_Y.n_rows != (unsigned int) p_Y || Omega_Y.n_cols != 2) {
    Rcpp::stop("Invalid conditional data, prediction, or sample-space dimensions.");
  }
  if (k_X < 1 || k_Y < 1) Rcpp::stop("Maximum resolutions must be positive.");


  if (k_X > MAXVAR - 1) {
    Rcpp::warning("Maximum resolution for X %d exceeds limit %d; using %d.",
                  k_X, MAXVAR - 1, MAXVAR - 1);
    k_X = MAXVAR - 1;
  }

  if (k_Y > MAXVAR - 1) {
    Rcpp::warning("Maximum resolution for Y %d exceeds limit %d; using %d.",
  ...
  if (nobs_new > 0) {
    ...
    const double density_scale = arma::prod(a_Y);
    for (size_t i = 0; i < predictive_densities_vec.size(); i++) {
      predictive_densities_vec[i] *= density_scale;
    }
  }
