...
CondGBT::CondGBT(Mat< unsigned int > X, Mat< unsigned int> Y, int k_X, int k_Y, int p_X, int p_Y,
              ...
              GBT(X,k_X,p_X,rho0_X,rho0_mode_X,1,lognu_lowerbound_Y,lognu_upperbound_Y,n_grid_Y,1,beta_Y),
              gbt_ptrs(NULL), k_Y(k_Y), p_Y(p_Y), n_s_Y(n_s_Y),
              rho0_Y(rho0_Y), rho0_mode_Y(rho0_mode_Y), tran_mode_Y(tran_mode_Y) {

         // The base class constructor allocates an unconditional tree. The
         // conditional model needs different storage, so release it first.
         GBT::clear();
         nobs = X.n_rows;
         init(X,Y);
}
...
void CondGBT::init(Mat< unsigned int > X, Mat< unsigned int > Y) {
  ...
  for (i=0; i <= k; i++) {
    ...
    models[i] = new double[
      ((unsigned long long) modelscount[i] * numnodevar) << i
    ];
  ...
  if (gbt_ptrs != NULL) {
    for (int i = 0; i <= k; i++) {
      for (unsigned long long j = 0;
           j < ((unsigned long long) modelscount[i] << i); j++) {
        delete gbt_ptrs[i][j];
      }
      delete [] gbt_ptrs[i];
    }
...
void CondGBT::sample_subtree(INDEX_TYPE& I,int level,int s) {
  ...
  if (level == k || log(u) <= logrho_vec[n_s]) { // pruned?
    ...
    sample_nodes.push_back(make_pair(
      make_pair(I, level),
      make_pair(std::numeric_limits<double>::infinity(), 0)
    ));
...
int CondGBT::update_node(double *NODE_CURR, int level, INDEX_TYPE I) {
  ...
  if (NODE_CURR[0] > 0) {
    if (NODE_CURR_GBT_PTR[0] == NULL) {
      Rcpp::stop("Internal error: missing response tree for a non-empty predictor node.");
    }
...
void CondGBT::add_data_to_subtree(INDEX_TYPE I, int level, int x_curr, int part_count,
  ...
  if (NODE_CURR[0] == 1) {
    ...
    NODE_CURR_GBT_PTR[0] = new GBT(conv_to< Mat< unsigned int> >::from(obs_Y.t()),k_Y,p_Y,rho0_Y,rho0_mode_Y,tran_mode_Y,
                                   lognu_lowerbound,lognu_upperbound,n_grid,n_s_Y,beta); // need to specify initial values
  }
...
int CondGBT::update_subtree_add_new_data(INDEX_TYPE I, int level, int x_curr, int part_count, Col< unsigned int > new_obs_X, Col< unsigned int > new_obs_Y) {
  ...
  if (NODE_CURR_GBT_PTR[0] != NULL) {
    ...
    NODE_CURR_GBT_PTR[0] = new GBT(conv_to< Mat< unsigned int> >::from(new_obs_Y.t()),k_Y,p_Y,rho0_Y,rho0_mode_Y,tran_mode_Y,
                                   lognu_lowerbound,lognu_upperbound,n_grid,n_s_Y,beta);
    NODE_CURR_GBT_PTR[0]->update();
  }
